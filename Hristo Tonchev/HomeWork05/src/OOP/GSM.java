...
public class GSM {
    ...
    {
       for (int i =0;i<CallHistory.size();i++)//?????????????????????????????????????????CallHistory.Count??
        {
            if (CallHistory.get(i).equals(Duration))//?????????????????????????????????????????CallHistory[i].Duration == duration??????????
            {
        ...
        public void PrintCalls()
    {
        for (int i=0;i<CallHistory.size();i++)
        {
            ...
            System.out.println("Date: "+ (CallHistory.get(i).getDate()));//??????????????CallHistory[i].Date
            System.out.println("Time: "+ (CallHistory.get(i).getTime()));//????????????CallHistory[i].Time
            System.out.println("Diled pohne number: "+ (CallHistory.get(i).getPhoneNumber()));//?????????????????CallHistory[i].DialedPhoneNumber
            System.out.println("Duration: "+ (CallHistory.get(i).getDuration()));//????????????????????CallHistory[i].Duration
